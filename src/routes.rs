...
//! (`src/http/route_dto.rs` in infernal-law) field-for-field -- proven
//! first in `infernal-worker-simple`, reused here unchanged.
...
/// element is the oldest eligible route.
pub fn parse_eligible_routes(status: u16, body: &[u8]) -> Result<Vec<EligibleRoute>, ParserError> {
    if status != 200 {
        return Err(ParserError::UnexpectedStatus(status));
    }
...
mod tests {
    ...
    fn rejects_a_non_200_status() {
        assert!(matches!(
            ...
            Err(ParserError::UnexpectedStatus(503))
        ));
    }
    ...
    fn rejects_malformed_json() {
        assert!(matches!(
            ...
            Err(ParserError::MalformedResponse(_))
        ));
    }
}
