...
pub fn parse_claim_response(status: u16, body: &[u8]) -> Result<ClaimOutcome, ParserError> {
    match status {
        201 => {
        ...
        other => Err(ParserError::UnexpectedStatus(other)),
    }
}
...
pub fn parse_complete_response(status: u16, body: &[u8]) -> Result<CompleteOutcome, ParserError> {
    match status {
        200 => {
            let claim: WorkClaim = serde_json::from_slice(body)
                .map_err(|error| ParserError::MalformedResponse(error.to_string()))?;
