...
pub fn parse_subscription_list(status: u16, body: &[u8]) -> Result<Vec<Subscription>, ParserError> {
    if status != 200 {
...
pub fn parse_create_subscription_response(
    status: u16,
    body: &[u8],
) -> Result<Subscription, ParserError> {
    if status != 201 {
        return Err(ParserError::UnexpectedStatus(status));
    }
    serde_json::from_slice(body).map_err(|error| ParserError::MalformedResponse(error.to_string()))
}
